...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if 'bce' in args.loss :
            num_sampling = int(num_classes * args.sampling_rate)
            full_output = outputs
            new_outputs, new_bce_targets = sampling_for_cifar100(outputs, targets, num_sampling, num_classes=num_classes)
            temp2 = args.bce_scale * F.binary_cross_entropy_with_logits(new_outputs, new_bce_targets)
