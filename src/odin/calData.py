...
########################################In-distribution###########################################
    for j, data in enumerate(testloader10):
        ...
        # inputs = Variable(images.cuda(CUDA_DEVICE), requires_grad = False)
        # outputs = net1(inputs)[0]
        outputs = net1(images)

        activated_output = F.softmax(outputs)
        max_output, pred = activated_output.max(1)
        f1.write("{}, {}, {}\n".format(temper, noiseMagnitude1, max_output[0].data.cpu().numpy()))
        # nnOutputs = activated_output.data.cpu().numpy()[0]
